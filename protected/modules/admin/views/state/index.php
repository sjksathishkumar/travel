...
<div class="breadcrumbs">
	<ul>
		...
		<li><?php echo CHtml::link('Manage States',array('#')); ?></li>
	</ul>
...
<div class="breadcrumbs" id="breadcrumbs-msg">
	<?php  if( (Yii::app()->User->hasFlash('addStateSuccess')) || (Yii::app()->User->hasFlash('updateStateSuccess')) || (Yii::app()->User->hasFlash('deleteStateSuccess'))){ ?>
		<ul>
			<?php
				if(Yii::app()->User->getFlash('addStateSuccess'))
				{
					echo '<li><span class="readcrum_without_link_success">'.ADD_STATE_SUCCESS.'</li>';
				}else if(Yii::app()->User->getFlash('updateStateSuccess'))
				{
					echo '<li><span class="readcrum_without_link_success">'.UPDATE_STATE_SUCCESS.'</li>';
				}
			?>						
	      </ul>
	<?php } ?>
	...
	/*Yii::app()->clientScript->registerScript('create', "
							...
							$('.container-fluid').on('submit','.add-form form',function(e){
								...
								$.post($(this).attr('action'),$(this).serialize(),function(data){
									$('#state-grid').html($(data).find('#state-grid').html());
...
<div class="row-fluid">
	<div class="span12 margin_top20">
		<?php echo CHtml::link('Add State',array('create'),array('class'=>'add-button btn btn-primary')); ?>
...
<div class="row-fluid">
	<div class="span12">
		<div class="box box-color box-bordered">
			<div class="box-title">
				<h3><i class="icon-table"></i>State List</h3>
			</div>
			<div class="box-content nopadding">
				<form action="" name='state-grid-list-form' id='state-grid-list-form'>					
					<?php $this->widget('zii.widgets.grid.CGridView', array(
						...
						'columns'=>array(
							...
							array('name'=>'fkCountryID',
								'value'=>'$data->country->countryName',								
								'htmlOptions'=>array('style'=>'text-align:center'),
							),	
							array('name' => 'stateStatus',
                                'type' => 'raw',
                                'value' => 'CommonFunctions::statusFurmate($data->stateStatus)',
                                'htmlOptions' => array('style' => 'text-align:center'),
                            ),
							array(
								'header'=>'Action',
					...
					<div style="padding-left:1%">
						<select id="state-status" name="state-status" class="input-large nomargin select2-me" onchange="stateMultipleAction();">
...
<script type="text/javascript">
	...
	*/
	...
	function stateMultipleAction()
	{
		var checked_num = $('input[name="state-grid_c1[]"]:checked').length;
		if (!checked_num) {
			alert('Please select atleast one.');
		}	
		else
		{
			if ($('#state-status').val()=='Select') {
				alert('Please Select valid option');
			}else{
				var status = confirm('Are you sure to change the status of selected record(s)?');   
				if(status)			
				{
					var data=$("#state-grid-list-form").serialize();
					$.ajax({
					type: 'POST',
					url: '<?php echo CController::createUrl("state/changeStatus"); ?>',
					data:data,
					success:function(data){
						     if(data)
							{ 
								var statusMsg = 'State(s) '+data+' successfully.';
		...
		$("#state-status option[value='Select']").attr("selected","selected");
		$('#state-status.select2-me').select2('val','Select');
	}
	   ...
	   $('#resetVal').live('click', function(e){
			...
			$.fn.yiiGridView.update('state-grid', {url:'<?php echo CController::createUrl("/admin/state"); ?>'});
		});
